...
uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((aligned (32))) __attribute__ ((section (".exsramtext")));
//__GNU_ALIGN(32) uint8_t mem1base[MEM1_MAX_SIZE];													//内部SRAM内存池
//__GNU_ALIGN(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(0X68000000)));					//外部SRAM内存池

//内存管理表
uint16_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__ ((section (".exsrammaptext")));	//外部SRAM内存池MAP
